...
class ProSan:
    ...
    def _token_importance(self, prompt, content):
        ...
        output = output.lower().replace("<pad>", "").replace("</s>", "").replace(".", "").strip(",._\"\'-+=!?()&^%$#@:\\|\{\}[]<>/`\n\t\r\v\f ").split(" ")[0]
        label = mapping.get(output[0], -1)
        ...
        outputs = self.model(inputs_embeds=embeddings, attention_mask=query_label_masks, labels=labels)
    ...
    def sanitize(self, dataset):
        ...
        with open("../results/MedQA_sanitized_content.pkl", "wb") as f:
...
with open("../dataset/MedQA.pkl", 'rb') as f:
    dataset = pickle.load(f)
config_logger("../results/MedQA_privacy_result")
sanitizer = ProSan(model, tokenizer, bert_model, bert_tokenizer, "<<SYS>>\n{prompt}\n<</SYS>>{content}\nAnswer:", f"Please answer with one of the option in the bracket.")
